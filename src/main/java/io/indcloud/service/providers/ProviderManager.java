    ...
    public String sendSms(String to, String message) {
        ...
        try {
            ...
            String messageId = circuitBreakerService.executeWithCircuitBreaker(
                ...
                () -> providers.get(selectedProvider).sendSms(to, message)
            );
        ...
        for (String nextProvider : availableProviders) {
            try {
                ...
                String messageId = circuitBreakerService.executeWithCircuitBreaker(
                    ...
                    () -> providers.get(nextProvider).sendSms(to, message)
                );
    ...
    private static class ProviderMetrics {
        ...
        public Map<String, Object> toMap() {
            return Map.of(
                ...
                "averageResponseTime", String.format("%.2fms", getAverageResponseTime()),
                "costEfficiency", String.format("$%.4f", getCostEfficiency()),
                "healthy", isHealthy(),
                "lastSuccess", new Date(lastSuccessTime).toString(),
                "lastFailure", lastFailureTime > 0 ? new Date(lastFailureTime).toString() : "Never",
                "brazilianScore", String.format("%.2f", calculateBrazilianScore()),
                "recentErrors", recentErrors.size() > 0 ? recentErrors.subList(0, Math.min(3, recentErrors.size())) : List.of("None")
            );
        }
    }
}